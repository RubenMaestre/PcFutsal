// components/FullClassificationShell.tsx
"use client";
...
function RachaDotsFull({
  ...
  let arr: string[] = [];
  if (Array.isArray(racha)) {
    arr = racha.map((x) => String(x).toUpperCase());
  } else if (typeof racha === "string" && racha.trim() !== "") {
    arr = racha.trim().toUpperCase().split("");
  }
  ...
  return (
    <div className="flex flex-wrap gap-1 mt-1">
      {arr.map((res, idx) => {
        ...
        let bgClass = "bg-[var(--color-error)]";
        if (code === "V") {
        ...
        return (
          <span
            ...
            title={`Partido ${idx + 1}: ${code}`}
            onClick={onClickDot ? () => onClickDot(idx, code) : undefined}
          />
        );
      })}
    </div>
...
export default function FullClassificationShell({
  rows,
  dict,
  scope,
  setScope,
  ...
  const prevPosByClub = React.useMemo(() => {
    ...
    prevRows.forEach((raw, idx) => {
      const clubKey = raw.club_id ?? raw.id ?? raw.nombre ?? raw.team_name;
      if (clubKey) m[clubKey] = idx + 1;
    });
    return m;
  }, [prevRows]);
          ...
          <tbody>
            {sortedRows.length === 0 ? (
              ...
              sortedRows.map((row, idx) => {
                ...
                const prevPos = clubKey ? prevPosByClub[clubKey] : undefined;
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <RachaDotsFull
                            ...
                            onClickDot={(index, code) => {
                              console.log("Click racha", index, code, row.nombre);
                            }}
                          />
                        </div>
                      </Link>
